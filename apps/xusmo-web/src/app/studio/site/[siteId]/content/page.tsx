...
export default function ContentEditorPage() {
  ...
  const [isEngineSite, setIsEngineSite] = useState(false);
  ...
  const fetchPages = useCallback(() => {
    ...
    fetch(`/api/portal/sites`)
      ...
      .then((sites) => {
        ...
        setPages(sitePages);
        setIsEngineSite(!!site?.hasDesignDocument);
  ...
  return (
    <div style={{ fontFamily: "'Inter',-apple-system,sans-serif", display: "flex", flexDirection: "column", minHeight: "calc(100vh - 170px)" }}>
      {/* Engine-site info banner */}
      {isEngineSite && (
        <div style={{
          display: "flex", alignItems: "center", gap: 8,
          padding: "10px 24px",
          background: `${C.blue}08`, borderBottom: `1px solid ${C.blue}20`,
          fontSize: 13, color: C.blue,
        }}>
          <Info size={15} style={{ flexShrink: 0 }} />
          <span>
            This site uses the Visual Editor for layout and design.
            Text edited here updates headlines and SEO fields.{" "}
            <Link href={`/studio/site/${siteId}/design`} style={{ fontWeight: 600, textDecoration: "underline", color: C.blue }}>
              Open Design Studio
            </Link>
          </span>
        </div>
