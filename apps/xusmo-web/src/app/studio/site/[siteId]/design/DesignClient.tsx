...
// =============================================================================
// Design Client — Tabbed layout: Theme Gallery + Customize panel
// =============================================================================
...
type Tab = "gallery" | "customize" | "sections";
...
export default function DesignClient({
  ...
  // Engine sites default to Sections tab; WordPress sites default to Gallery
  const isEngineSite = site.hasDesignDocument && !site.wpUrl;
  const [activeTab, setActiveTab] = useState<Tab>(isEngineSite ? "sections" : "gallery");
  ...
  const refreshPreview = useCallback((delayMs = 2000) => {
    setIframeLoading(true);
    setTimeout(() => setIframeKey((k) => k + 1), delayMs);
  }, []);

  // Listen for AI agent changes to auto-refresh the preview
  useEffect(() => {
    const handler = () => refreshPreview(2500);
    window.addEventListener("xusmo-design-refresh", handler);
    return () => window.removeEventListener("xusmo-design-refresh", handler);
  }, [refreshPreview]);
  ...
  const applyPreset = async (presetId: string) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setPresetToast("Failed to apply style. Please try again.");
        setTimeout(() => setPresetToast(null), 5000);
      }
    } catch {
      setPresetToast("Network error. Please try again.");
      setTimeout(() => setPresetToast(null), 5000);
    }
  ...
  return (
    ...
    >
      ...
      >
        <Palette size={20} color={C.accent} />
        <h1 style={{ fontSize: 16, fontWeight: 700, color: C.text, margin: 0, marginRight: 20 }}>
          Design
        </h1>

        {/* Tab buttons */}
        <div style={{ display: "flex", gap: 4, background: C.surfaceAlt, borderRadius: 10, padding: 3 }}>
          {TABS.map((tab) => {
        ...
        {(() => {
          ...
          <a
            ...
            onClick={(e) => {
              ...
              a.href = `${previewHref}${previewHref.includes("?") ? "&" : "?"}cb=${Date.now()}`;
            }}
            target="_blank"
      ...
      <div style={{ flex: 1, overflow: "hidden", display: "flex" }}>
        ...
        {activeTab === "gallery" && (
          <div style={{ flex: 1, overflow: "hidden" }}>
            <ThemeGallery
              siteId={site.id}
              currentArchetype={site.archetype}
              activeThemeId={site.themePoolEntryId}
              onThemeApplied={() => {
                router.refresh();
                refreshPreview();
              }}
            ...
            >
              ...
              >
                ...
                <div style={{ fontSize: 12, color: C.dim, display: "flex", alignItems: "center", gap: 6 }}>
                  ...
                  {industry
                    ? `Based on ${industry.displayName} industry defaults.`
                    : "Set from your build preferences."}
                </div>
              </div>
                  ...
                  <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                    ...
                    <div style={{ textAlign: "left" }}>
                      ...
                      <div style={{ fontSize: 12, color: C.dim }}>
                        Fine-tune your site&apos;s color palette.
                      </div>
                    </div>
                  </div>
                ...
                >
                  <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                    ...
                    <div style={{ textAlign: "left" }}>
                      ...
                      <div style={{ fontSize: 12, color: C.dim }}>
                        Curated heading + body font combinations.
                      </div>
                    </div>
                  </div>
              ...
              >
                ...
                {presetToast && (() => {
                  const isError = presetToast.includes("Failed") || presetToast.includes("error");
                  const toastColor = isError ? C.red : C.green;
                  return (
                    <div style={{ display: "flex", alignItems: "center", gap: 6, padding: "8px 12px", background: `${toastColor}15`, border: `1px solid ${toastColor}30`, borderRadius: 6, marginBottom: 14, fontSize: 12, color: toastColor }}>
                      {isError ? <AlertTriangle size={14} /> : <CheckCircle2 size={14} />} {presetToast}
                    </div>
                  );
                })()}
                  ...
                  <div style={{ padding: "0 20px 20px" }}>
                    ...
                    <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginTop: 12 }}>
                      <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                        <span style={{ fontSize: 11, color: customCss.length > 5000 ? C.red : C.dim }}>
                          {customCss.length} / 5,000
                        </span>
                        {lastUpdated && (
                      ...
                      <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                        {cssError && (
                          <span style={{ display: "flex", alignItems: "center", gap: 4, fontSize: 12, color: C.red }}>
                            <AlertTriangle size={14} /> {cssError}
                          </span>
                        )}
                        {cssSaved && (
                    ...
                    <div style={{ display: "flex", alignItems: "center", gap: 8, marginTop: 12, padding: "8px 12px", background: `${C.amber}10`, border: `1px solid ${C.amber}30`, borderRadius: 6 }}>
                      ...
                      <span style={{ fontSize: 12, color: C.amber }}>
                        Incorrect CSS may break your site&apos;s appearance.
                      </span>
                    </div>
                  </div>
                )}
                ...
                <div style={{ height: 40, background: C.surface, borderBottom: `1px solid ${C.border}`, display: "flex", alignItems: "center", justifyContent: "space-between", padding: "0 12px", flexShrink: 0 }}>
                  ...
                  <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                    ...
                    <a
                      ...
                      style={{ color: C.muted, padding: 4, display: "flex", alignItems: "center" }}
                    >
