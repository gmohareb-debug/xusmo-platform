// Inline SVG icons for social platforms — avoids emoji rendering issues
const SOCIAL_ICONS = {
  ...
  return (
    <footer className="footer">
      <div className="footer__top">
        {(logo || tagline || address) && (
          <div className="footer__brand">
            {logo && <span className="footer__logo">{logo}</span>}
          ...
          <div className="footer__columns">
            {columns.map((col, i) => (
              <div key={i} className="footer__column">
                {col.title && <h4 className="footer__column-title">{col.title}</h4>}
                {col.links?.map((link, j) => (
                  <a key={j} href={link.href || '#'} className="footer__column-link">
                    {link.label}
                  </a>
                ))}
              </div>
      ...
      <div className="footer__bottom">
        {copyrightText && <p className="footer__text">{copyrightText}</p>}
        {links.length > 0 && (
